...
| `assets` | Bildvorrat — hochgeladene Rahmen-, Icon- und Artwork-Dateien, hinter der Anmeldung ausgeliefert (ADR-015). UI-Screen `features/assets/asset-library/`, Route `/assets`, Sidebar-Eintrag „Bildvorrat" |
...
| `print-project` | Druckprojekt-Verwaltung (genau ein Projekt, ADR-024), Druckbogen-Export (PDF/PNG). Backend, Store/Route/Seitenspalten-Eintrag, Bildschirm (Positionsliste, Druckoptionen, „Drucken"-Knopf in der Kartenliste), Bogen-Geometrie/Vorschau, Export als PDF und PNG sowie der Schärfe-Hinweis vor dem Export stehen — Meilenstein 5 ist abgeschlossen |
| `meta` | Nur Backend, kein Frontend: `MetaController`/`MetaService` liefern `GET /api/meta` — Enums/Grenzwerte der laufenden API aus den vorhandenen Validatoren/Services, als Schema-Quelle für den lokalen MCP-Server (Meilenstein 6) |
...
frontend/src/app/
  ...
  features/
    assets/
      asset-library/       ← Route /assets „Bildvorrat": drei Reiter (Rahmen/Icons/Artwork),
                             Liste mit Thumbnail + inline umbenennbarem Namen + Löschen,
                             Dropzone mit sequenziellem Multiupload (Warteschlange lokal in der
                             Komponente, Store kennt nur eine Datei gleichzeitig)
    auth/
      login/               ← Anmeldeseite
    ...
    cards/
      ...
      card-editor/         ← Routen /cards/new und /cards/:id: linke Formularspalte (Name,
                             ...
                             denselben Stand für das Vorschaubild der Kachel, der
                             Herunterladen-Knopf ruft ihn in Druckauflösung). `card-fields.ts`
  ...
  shared/
    ...
    canvas/            ← alles, was mit Konva zeichnet — Feature-Komponenten binden nur Daten
      ...
      rendering/        ← reine Zeichenregeln ohne Konva-Abhängigkeit: `layer.ts` (die fünf
                           ...
                           `sheet-layout.ts` (wo eine Karte auf dem A4-Bogen liegt:
                           `buildSheets()`, `sheetMarks()`, `mmToPx()` — die eine Rechnung
                           in Millimetern, aus der Vorschau, PDF und PNG gleichermaßen
                           schöpfen),
                           ...
                           **gespeicherte** Karte in genau das; die Entwurfs-Fassung im
                           ...
                           Ankerpunkt), `image-sharpness.ts` (`lowResolutionLayers()` — welche
                           Bildebenen einer Karte unter 300 dpi liegen, aus den
                           Originalmaßen der Datei und der gezeichneten Größe), `apply-transform.ts`
...
`print-project/` ist mit Meilenstein 5 vollständig (Bildschirm, Bogen-Vorschau, Export als
PDF und PNG, Schärfe-Hinweis). `cards/` (inkl. „Drucken"-Knopf in Grid- und Tabellenansicht)
und `templates/` sind ebenfalls vollständig (Liste, Editor, Vorschau).
