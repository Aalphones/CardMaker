...
- [x] `docs/decisions/005-client-side-rendering.md` anlegen.
- [x] Kontext: Das Konzeptdokument sah serverseitiges Rendern vor. Konva zeichnet die Karte
...
- [x] Optionen: (a) nur Browser, (b) nur Server, (c) beides.
- [x] Entscheidung: **nur Browser**. Konva-Bühne mit erhöhtem Pixelverhältnis exportieren.
...
- [x] `docs/decisions/006-deployment-und-abhaengigkeiten.md` anlegen.
- [x] Kontext: Auf der Entwicklungsmaschine gibt es weder PHP noch Composer noch MySQL noch
...
- [x] Optionen: (a) PHP und Composer lokal installieren, (b) `vendor/` von einem
      Bau-Durchlauf erzeugen lassen, (c) Backend ohne Composer-Abhängigkeiten schreiben.
- [x] Entscheidung: **(c)**. Fünf vorgesehene Bibliotheken werden ersetzt durch rund 200
      Zeilen eigenen Code:
...
- [x] Ausdrücklich festhalten: **Nichts Kryptografisches wird selbst gebaut.** Der einzige
...
- [x] Hochladen: ein Windows-Skript `deploy.cmd` im Projektstamm, Zugangsdaten in
      `deploy.env` (nicht im Git). Kein GitHub-Actions-Durchlauf, `.github/` wird entfernt.
- [x] Konsequenzen: Jede Backend-Änderung braucht einen Hochlade-Lauf; es gibt keine
...
- [x] `docs/decisions/007-character-attributes.md` anlegen.
- [x] Kontext: Templates greifen auf Werte wie `character.element` zu. Ein generischer
      Kartengenerator kann diese Felder nicht fest verdrahten — jedes Kartensystem hat andere.
- [x] Optionen: (a) feste Spaltenliste, (b) frei benannte Attribute als JSON-Feld,
      (c) eigene Attribut-Tabelle mit Definition und Werten.
- [x] Entscheidung: **(b)**. Kern fest (`name`, `description`), alles Weitere in einer
...
- [x] Konsequenzen: keine Schema-Änderung bei einem neuen Kartensystem; Attributwerte sind
...
- [x] `docs/decisions/008-opaque-tokens.md` anlegen.
- [x] Kontext: ADR-004 sah JWT für den Browser und dauerhafte Zugriffstoken für Skripte vor —
...
- [x] Optionen: (a) JWT-Bibliothek doch einbinden und `vendor/` von Hand pflegen,
      (b) JWT selbst implementieren, (c) beide Tokenarten als Zufallswerte in der Datenbank.
- [x] Entscheidung: **(c)**. Anmeldung erzeugt 32 Zufallsbytes, gespeichert wird nur der
...
- [x] Konsequenzen ehrlich benennen:
...
- [x] `docs/decisions/004-jwt-plus-pat-auth.md`: Status auf
...
- [x] `docs/decisions/009-keine-automatisierten-tests.md` anlegen.
- [x] Kontext: `docs/conventions/testing.md` schrieb Unit-Tests für Fachlogik vor, mit dem
...
- [x] Optionen: (a) Testpflicht für Fachlogik wie ursprünglich geplant, (b) Tests nur für den
      Rechenkern ab dem Rendering-Plan, (c) gar keine Tests.
- [x] Entscheidung: **(c)** für dieses Fundament. Geprüft wird über den Abnahme-Rundgang im
      Plan.
- [x] Konsequenzen: `docs/conventions/testing.md` entfällt; `docs/conventions/dod.md` verliert
      die automatisierte Ebene und wird zur reinen Handprüfung; kein Testwerkzeug im Frontend,
      kein PHPUnit im Backend.
- [x] **Wiedervorlage ausdrücklich festhalten:** Vor dem Rendering-Plan (Meilenstein 4) wird
...
- [x] `docs/decisions/010-semantic-css-statt-tailwind.md` anlegen.
- [x] Kontext: `docs/conventions/tailwind.md` verbot Utility-Klassen im Template bereits
...
- [x] Optionen: (a) Tailwind nur als `@theme`-Token-Quelle behalten (bisheriger Stand),
...
- [x] Entscheidung: **(b)**. Kein `tailwindcss`, kein `@tailwindcss/postcss`, keine
...
- [x] Konsequenzen:
  ...
  - Eine Zeile weniger Build-Tooling, ein Abhängigkeit weniger zu pflegen — passt zur Linie aus
    ...
    betrifft).
- [x] `AGENTS.md` → Conventions-Index: Zeile „Tailwind" auf „CSS / Styling" →
      `docs/conventions/css.md` ändern.

### Testgerüst und Bau-Automatik entfernen

- [x] `docs/conventions/testing.md` löschen.
- [x] `AGENTS.md` → Conventions-Index: Zeile „Testing" entfernen.
- [x] `docs/conventions/dod.md` überarbeiten: den Abschnitt „Zwei Ebenen" auf eine Ebene
...
- [x] `docs/conventions/php.md`: Abschnitt „Rendering-Layer" entfernen, `Rendering/` aus dem
...
- [x] `.github/` samt Inhalt löschen.
- [x] `package.json`: aus `lint-staged` die Zeile für `backend/**/*.php` entfernen — sie ruft
...
- [x] `docs/conventions/linting.md`: prüfen, ob dort PHP CS Fixer oder ein Bau-Durchlauf
...
- [x] `docs/PROJECT.md` → Stack-Tabelle:
...
- [x] `docs/PROJECT.md` → Meilenstein 4 umformulieren: „Rendern in Druckauflösung im Browser
      (ADR-005)".
- [x] `docs/PROJECT.md` → „Offene Fragen": die Frage nach client- oder serverseitigem Export
...
- [x] `docs/PROJECT.md` → Stack-Abschnitt: den Absatz „Offene Stack-Frage" über
      Imagick-Verfügbarkeit entfernen, ein Satz zu ADR-005 stattdessen.
- [x] `docs/conventions/stack.md`: Backend-Bibliotheken, Tooling- und Styling-Zeile
      ...
      entfernen.
- [x] `docs/conventions/angular.md`: Stack-Tabelle Zeile „Styling" auf „Semantic CSS (BEM +
      Custom Properties)" ändern, Abschnitt „CSS / Styling" verweist auf `css.md` statt
      `tailwind.md`.
- [x] `README.md` und `AGENTS.md` → Tech-Stack-Kurzfassung: „Tailwind v4" aus der
      Frontend-Aufzählung streichen.
- [x] `docs/code-map.md`: `Rendering/` aus dem Backend-Layout entfernen, `Support/` ergänzen;
      im Frontend-Layout unter `shared/canvas/` den Unterordner `rendering/` ergänzen.
- [x] `docs/conventions/mcp.md` überfliegen und Stellen korrigieren, die serverseitiges
      Rendern oder JWT voraussetzen.

## Report-Back

Sechs ADRs (005–010) angelegt, ADR-004 auf „Abgelöst durch ADR-008" gesetzt und im Index
nachgezogen. Testgerüst und Bau-Automatik entfernt: `docs/conventions/testing.md` gelöscht,
`.github/` gelöscht, `dod.md` auf reine Handprüfung gekürzt, `php.md` ohne
Rendering-Layer-Abschnitt (Critical Rules 4+5 neu: kein Rendering, keine
Composer-Abhängigkeiten), `package.json` ohne Tailwind/PostCSS-Devdependencies und ohne den
PHP-CS-Fixer-Lint-Staged-Eintrag. `tailwind.md` durch neues `css.md` ersetzt (BEM + SCSS +
Custom-Properties, kein Tailwind-Bezug mehr). `PROJECT.md`, `stack.md`, `angular.md`,
`code-map.md`, `README.md`, `AGENTS.md` auf den neuen Stand gebracht (kein serverseitiges
Rendering, kein Composer, kein JWT, kein Tailwind).

**Chesterton's Fence / Nebenfund:** `docs/conventions/git-workflow.md` dokumentierte im
Pre-Commit-Hook-Abschnitt noch `backend/**/*.php` → `php-cs-fixer fix` — derselbe
Composer-Widerspruch wie in `package.json`, aber nicht in der Aufgabenliste dieser Phase
gelistet. Mitgefixt (Zeile ersetzt durch „kein Backend-Formatter, ADR-006"), da er sonst eine
Bau-Automatik behauptet hätte, die es nicht mehr gibt.

`docs/conventions/mcp.md` geprüft — enthielt weder Verweise auf serverseitiges Rendering noch
auf JWT (nur „gleiche PAT-Auth wie das Web-Frontend", was mit ADR-008 kompatibel bleibt).
Keine Änderung nötig.

`docs/conventions/linting.md` geprüft — enthielt keine Erwähnung von PHP CS Fixer oder einem
Bau-Durchlauf. Keine Änderung nötig.
