...
- [ ] `backend/src/Migrations/M010AddPreviewImages.php` anlegen: zwei `ALTER TABLE`-Befehle,
...
- [ ] `COLUMNS` um `preview_file_name`, `preview_updated_at` erweitern.
- [ ] Die Listen-Abfrage (`allSummaries()` bzw. das Gegenstück in `CardRepository`):
...
- [ ] `formatSummary()` und `format()`: `previewUpdatedAt` ergänzen
...
- [ ] Neu: `findPreviewFileName(int $id): ?string` — `null`, wenn Datensatz fehlt oder kein
      Bild gesetzt ist.
- [ ] Neu: `updatePreview(int $id, string $fileName): ?string` — setzt beide Spalten
...
- [ ] Neu: `clearPreview(int $id): void` — beide Spalten auf `NULL`.

### Gemeinsamer Ablage-Baustein

- [ ] `backend/src/Services/PreviewImageStorage.php` — kennt **keine** Tabelle, nur den
...
- [ ] `backend/src/Services/PreviewImageUploadException.php` — analog zu
...
- [ ] `backend/src/Services/TemplatePreviewService.php` — Konstruktor
...
- [ ] `backend/src/Services/CardPreviewService.php` — dasselbe gegen `CardRepository`, plus
...
- [ ] `backend/src/Controllers/TemplatePreviewController.php` und
...
- [ ] `TemplateService::delete()`: Bild entfernen — **erst nach** der 409-Prüfung auf
      benutzende Karten, sonst ist das Bild weg, obwohl das Template bleibt.
- [ ] `CardService::delete()`: Bild entfernen, dort wo schon
      `CardImageService::deleteAllForCard()` gerufen wird.
- [ ] `CardService::duplicate()`: `CardPreviewService::duplicateFor()` rufen, direkt neben dem
      bestehenden Aufruf für die Kartenbilder.
- [ ] `backend/public/index.php`: zwei `PreviewImageStorage` bauen
...
- [ ] `backend/uploads/.gitignore` prüfen: die neuen Unterordner dürfen nicht mitcommittet
      werden (so wie `uploads/cards/` behandelt wird).

### Doku

- [ ] `docs/decisions/021-vorschaubilder.md` — Kontext (Listen zeigten nur Metadaten),
...
- [ ] `docs/models.md` — die neuen Spalten an `templates` und `cards`.
- [ ] `docs/routes.md` — die vier neuen Endpunkte, inkl. `previewUpdatedAt` an den bestehenden
      Template- und Karten-Antworten.
- [ ] `docs/code-map.md` — Zeilen `templates` und `cards` um die Vorschau-Ablage ergänzen.
- [ ] `STATE.md` — Phase 1 abgehakt, nächster Schritt Phase 2.
