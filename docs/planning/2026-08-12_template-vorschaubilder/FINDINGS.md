...
- [x] → Phase 3: Nach dem Speichern läuft das Hochladen des Bildes **nebenher** — es ist noch
      ...
      Eingearbeitet: `previewUrl()`/`load()` hängen am `previewUpdatedAt` aus der Summary, nicht
      an einem lokalen Zeitpunkt.
- [x] → Phase 3 (und Karteneditor-Plan Phase 7): Der Dienst heißt
      ...
      demselben Muster.
      Eingearbeitet: Datei nach `shared/canvas/preview-upload.service.ts` verschoben, Klasse
      `PreviewUploadService`, `upload(kind, id, image)` für beide Sorten.
