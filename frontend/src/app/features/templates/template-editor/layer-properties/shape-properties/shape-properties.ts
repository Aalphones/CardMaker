...
@Component({
  ...
  imports: [GeometryFields, ColorField, AdvancedFields, ReactiveFormsModule],
...
export class ShapeProperties {
  ...
  protected rotationOrNull(layer: ShapeLayer): number | null {
    return layer.shape === 'line' ? null : layer.rotation;
  }

  protected cornerRadiusOrNull(layer: ShapeLayer): number | null {
    return layer.shape === 'rect' ? layer.cornerRadius : null;
  }
  ...
  protected emitCornerRadius(cornerRadius: number): void {
    this.layerChange.emit({ cornerRadius });
  }
